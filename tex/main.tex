...
\begin{document}
	...
	\begin{abstract}
		...
		为达成高精度的识别目标，本研究通过一套科学、可复现的机器学习流水线系统性地解决了模型选型问题：(1) 对多种传统机器学习模型（如随机森林、XGBoost）和两种前沿的Transformer音频识别模型（AST, PaSST）进行了全面的训练与评估；(2) 针对声源方向识别的独特挑战，我们创新性地对\textbf{PaSST模型进行了双声道改造}，使其能有效捕捉并利用决定声音空间感的关键特征。在此基础上，我们开发了一套基于Flask应用工厂模式的、功能完备的Web应用，集成了用户管理、数据加密、实时推理接口以及智能模型缓存等功能，实现了从模型到服务的无缝衔接。作为项目工程实践的最终环节，该Web服务被成功部署于一个由\textbf{TESLA V100 GPU}提供算力支持的虚拟化环境中，完成了从开发到部署的全流程闭环。
		...
		综上，本项目不仅交付了一个在关键任务上性能优越的深度学习模型，还完成了一次从模型开发、科学评估到最终服务化部署的\textbf{全栈系统工程实践}。
	...
	\begin{itemize}[leftmargin=*, topsep=0.5ex, itemsep=0.5ex]
		\item \textbf{在系统工程实践上：} 我们并未止步于命令行工具，而是构建了一套从模型训练到服务部署的\textbf{全栈应用}。该应用以后端Flask应用工厂与前端现代化UI为特色，集成了用户管理、实时推理API与功能强大的管理员后台，实现了AI能力的完整产品化落地。
	...
	\begin{itemize}[leftmargin=*, topsep=0.5ex, itemsep=0.5ex]
		...
		\item \textbf{进阶要求：} 本项目不仅完成了Level B要求的，从原始MP3音频出发，自主编写脚本提取 \textbf{MFCC 特征}，更进一步探索并实现了：(1) 基于 \textbf{Transformer} 架构的深度学习模型；(2) 一套完整的、从模型训练到服务化部署的全栈Web应用。
	\end{itemize}
	...
	该架构的核心优势在于：
	\begin{itemize}[leftmargin=*, topsep=0.5ex, itemsep=0.5ex]
		\item \textbf{职责分离：} AI模型开发（位于\texttt{src/}目录）与Web应用服务（位于\texttt{app/}目录）可以被独立地开发、测试和迭代，互不干扰。
...
\subsection{部署环境与资源占用分析}\label{s:5.4}

\subsubsection{部署环境配置}
本项目答辩所用的在线服务部署于一个虚拟化服务器环境中，其具体的软硬件配置如下：
\begin{itemize}[leftmargin=*, topsep=0.5ex, itemsep=0.5ex]
	\item \textbf{物理主机与虚拟化}: 系统运行在一台 \textbf{Dell PowerEdge R730xd (13代)} 服务器上，该服务器搭载了\textbf{两颗 Intel Xeon E5-2682v4 CPU }，并通过 \textbf{Proxmox VE (PVE)} 平台进行虚拟化管理。
	\item \textbf{虚拟机配置}: 我们的部署环境是一个独立的虚拟机，运行 \textbf{Ubuntu Server 24.04} 操作系统，并为其分配了 \textbf{32个虚拟CPU核心} 和 \textbf{32GB 内存}。
	\item \textbf{GPU配置}: 为满足项目的计算需求，该虚拟机被直通分配了一张 \textbf{TESLA V100 计算卡}，配备 \textbf{16GB SXM2 显存}。
	\item \textbf{软件环境}: 整个软件环境构建在 \textbf{Python 3.10} 之上，深度学习框架为 \textbf{PyTorch 2.9.1}，并使用 \textbf{CUDA Toolkit 12.6} 。
\end{itemize}

\subsubsection{缓存策略对静态资源占用的影响}
为了验证我们在 \texttt{InferenceService} 中设计的智能缓存策略的有效性，并精细化地分析不同类型模型的资源开销，我们对Web服务在多种缓存模式下的静态资源占用情况进行了对比测试。测试均在Web服务启动后、无任何用户交互的“空闲状态”下进行。

测试结果如表 \ref{tab:resource-usage} 所示。

\begin{table}[H]
	\centering
	\caption{不同缓存策略与模型类型下的静态资源占用对比}
	\label{tab:resource-usage}
	\begin{tabular}{@{}lcc@{}}
		\toprule
		\textbf{测试场景} & \textbf{内存 (RAM) 占用} & \textbf{显存 (VRAM) 占用} \\
		\midrule
		\texttt{selected} (无任何模型加载) & \textasciitilde{}0.7 GB & \textasciitilde{}0 GB \\
		\texttt{selected} (仅加载所有XGBoost模型) & \textasciitilde{}0.9 GB & \textasciitilde{}0.3 GB \\
		\texttt{selected} (仅加载所有PaSST模型) & \textasciitilde{}0.9 GB & \textasciitilde{}1.4 GB \\
		\texttt{all} (全部模型预加载) & \textasciitilde{}1.5 GB & \textasciitilde{}2.5 GB \\
		\bottomrule
	\end{tabular}
\end{table}

\textbf{分析与结论：}
从表中数据可以看到，不同模型类型与缓存策略对系统资源的占用有着显著差异：
\begin{itemize}[leftmargin=*, topsep=0.5ex, itemsep=0.5ex]
	\item \textbf{基础资源开销：} 在无任何模型加载的情况下，系统仅占用了约0.7GB的内存和0GB的显存，这部分主要源于Python解释器与Flask应用框架自身的开销。
	\item \textbf{模型类型的差异：} 两种类型的GPU加速模型在资源消耗上表现出巨大差异。加载所有基于树模型的XGBoost仅需额外约200MB内存和300MB显存。相比之下，加载所有基于Transformer的PaSST模型则需要额外约200MB内存和高达1.4GB的显存，后者对显存的开销更大。
	\item \textbf{缓存策略的价值：} \textbf{预加载}模式以1.5GB内存和2.5GB显存的静态资源占用，换取了所有模型的最快首次响应速度。而\textbf{按需加载}模式则提供了灵活性，允许管理员根据硬件限制（特别是在显存有限的设备上），选择性地预加载核心模型，从而在资源占用与响应性能之间做出动态权衡。
\end{itemize}
这一对比测试，说明了我们所设计的\textbf{智能缓存策略在真实部署环境中的灵活性}。
...
综上所述，本项目不仅圆满完成了课程设计的所有要求，更在系统架构、模型创新与工程完整性上进行了深入的探索与实现。我们认为，\textbf{当前系统作为一个课程设计产出，在性能与健壮性上尚属“玩具”级别，但它完整地实践并验证了一套从AI模型开发到Web服务部署的技术流程}。

回顾整个项目，核心成果可总结为以下三个方面：
\begin{itemize}[leftmargin=*, topsep=0.5ex, itemsep=0.5ex]
	\item \textbf{在系统工程实践上：} 我们成功设计并实现了一套\textbf{“双系统解耦”架构}，将模型开发与Web服务清晰分离。在此基础上，我们交付了一个包含用户认证、AES数据加密、动态模型管理和响应式UI的\textbf{全栈应用}，我们更将其\textbf{成功部署于虚拟化生产环境}中。这实现了从算法开发、应用构建到服务化部署的全流程闭环，标志着我们实践了一套完整的的AI应用工程。
...
\end{itemize}
	...
	为方便查阅更详尽的技术实现细节与项目背景，本项目的文档已在线发布：
