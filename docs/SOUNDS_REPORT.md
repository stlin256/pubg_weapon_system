...
**图表解读**:
四张图表共同揭示了一致的结论：无论以哪个核心指标衡量，**PaSST 和 AST 模型都稳居性能的第一梯队**。这有力证明了 Transformer 架构在本项目中的绝对优势，并为我们最终选择 PaSST 作为最优方案提供了坚实、多维度的数据支撑。
...
2.  **PaSST模型性能最佳，尤其是在方向识别上**:
    *   **经过双声道改造的PaSST** 在所有任务中都名列前茅，尤其是在最具挑战性的**方向 (direction)**识别任务上，其准确率达到了 **91.6%**，比次优的AST模型高出8个百分点，更是远超所有传统模型。
